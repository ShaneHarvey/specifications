...
The spec test format is an extension of `transactions spec tests </source/transactions/tests/README.rst>`_ with the following additions:

- ``writeConcern`` in the ``databaseOptions`` or ``collectionOptions`` may be an empty document to indicate a `server default write concern </source/read-write-concern/read-write-concern.rst#servers-default-writeconcern>`_. For example, in libmongoc:
