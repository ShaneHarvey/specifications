  ...
  - ``operations``: Array of documents, each describing an operation to be
    executed. Each document has the following fields:
      ...
      - ``result``: The return value from the operation, if any. If the
        ...
        or ``errorCode``, which is the expected server error code.
...
#. For each element in ``operations``:
   ...
   - If the result document has an "errorContains" field, verify that the
     ...
     an "errorCode" field, verify that the method threw a command failed
     exception or returned an error, and that the value of the "errorCode"
     field matches the received server error code.
